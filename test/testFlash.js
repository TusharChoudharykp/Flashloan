...
const { ethers, waffle, network } = require("hardhat"); // Import waffle correctly
const { fundContract } = require("../utils/utilities");

const { abi } = require("../artifacts/contracts/interfaces/IERC20.sol/IERC20.json");

const provider = network.provider; // Initialize provider correctly
...
describe("FlashLoan Contract", () => {
  ...
  beforeEach(async () => {
    ...
    const whale_balance = await ethers.provider.getBalance(BUSD_WHALE);
